...
void bottom_sonar_callback(const std_msgs::Float32& range_in) {
  bottom_sonar_dist = range_in.data;// - 1.0;
...
void vel_callback(const geometry_msgs::TwistStamped& vel_in) {
  observation[6] = 0.0;  //vel_in.twist.linear.x;
  observation[7] = 0.0;  //vel_in.twist.linear.y;
  observation[8] = 0.0;  //vel_in.twist.linear.z;
}  // vel_callback
...
int main(int argc, char* argv[]) {
  ...
  const unsigned int rp_filter_size = 4;
  ...
  std::vector<float> bottom_filter(5);
  for (int i = 0; i < 5; ++i) {
    bottom_filter[i] = 0.0;
  }
    ...
    if (!offboard_mode) {  // Quad is in manual or not flying
      quad.set_x(x0);
      geometry_msgs::Twist tmp_u;
      tmp_u.angular.x = 0.0;
      tmp_u.angular.y = 0.0;
      tmp_u.angular.z = 0.0;
      tmp_u.linear.z = -1.0;
      u_pub.publish(tmp_u);
    } else {  // Quad is in autonomous mode
      ...
      est_vel_pub.publish(est_vel_out);
  		...
  		if (new_lidar) {
  			...
  			static const float safety_dist = 4.15;
            ...
            range_list.push_back(safety_dist);
            tmp_point[0] = safety_dist * sin(theta_rel);
            tmp_point[1] = -safety_dist * cos(theta_rel);
            full_point_list.push_back(tmp_point);
          }
        ...
        if (new_bottom_sonar) { 
          new_bottom_sonar = false; 
          bottom_filter.pop_back();
          bottom_filter.insert(bottom_filter.begin(), bottom_sonar_dist);
          ROS_INFO("lidar_reading: %0.2f", bottom_sonar_dist);
        }
      }
...
*/
      ...
      static const float max_height = 0.5;
      static float err = 0.0;
      float height = std::accumulate(bottom_filter.begin(), bottom_filter.end(), 0.0) / 5.0f;
      ROS_ERROR("u_z: %0.2f", u_new[2]);
      err = max_height * (0.5 * u_new[2] + 0.5) - height; // quad.x_hat()[2];
      ROS_ERROR("z: %0.2f", height);
      ...
      ROS_ERROR("err: %0.2f, int: %0.2f", err, int_err);
